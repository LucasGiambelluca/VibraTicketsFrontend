...
export default function CustomerLogin() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (user.role === 'ADMIN' || user.role === 'ORGANIZER' || user.role === 'PRODUCER' || user.role === 'DOOR') {
        // Si es admin/staff, sugerir usar el login correcto
        message.info('Detectamos que eres parte del staff. Te redirigimos al panel de administración.');
        navigate('/admin');
      } else {
        // Cliente normal - redirigir a donde venía o al inicio
        navigate(from);
      }
        ...
        >
          ...
          <div style={{ textAlign: 'center', marginTop: 12 }}>
            <Text style={{ color: '#999', fontSize: '13px' }}>
              ¿Eres staff?{' '}
              <Link to="/adminlogin" style={{ color: '#999', textDecoration: 'underline' }}>
                Ingresá aquí
              </Link>
            </Text>
          </div>
